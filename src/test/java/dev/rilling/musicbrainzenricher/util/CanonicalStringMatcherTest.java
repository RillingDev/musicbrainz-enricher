...
class CanonicalStringMatcherTest {
	...
	void returnsSame() {
		...
		CanonicalStringMatcher canonicalStringMatcher = new CanonicalStringMatcher(canonicalValues, Set.of("-", " "), Collator.getInstance(Locale.ROOT));
	...
	void canonicalizes() {
		...
		CanonicalStringMatcher canonicalStringMatcher = new CanonicalStringMatcher(canonicalValues, Set.of("-", " ", " & ", " and "), Collator.getInstance(Locale.ROOT));
	...
	void respectsCollator() {
		...
		assertThat(new CanonicalStringMatcher(Set.of("yé-yé"), Set.of("-", " "), spanishCollator).canonicalize("ye ye")).contains("yé-yé");

		Collator caseInsensitiveCollator = Collator.getInstance(Locale.ROOT);
		caseInsensitiveCollator.setStrength(Collator.SECONDARY);
		assertThat(new CanonicalStringMatcher(Set.of("hip hop"), Set.of("-", " "), caseInsensitiveCollator).canonicalize("Hip Hop")).contains("hip hop");

		Collator caseSensitiveCollator = Collator.getInstance(Locale.ROOT);
		caseSensitiveCollator.setStrength(Collator.TERTIARY);
		assertThat(new CanonicalStringMatcher(Set.of("hip hop"), Set.of("-", " "), caseSensitiveCollator).canonicalize("Hip Hop")).isEmpty();
	}
